...
class ModulesTests: QuickSpec {
    ...
    override func spec() {
        describe("TweetTransformer") {
            it("should be able to handle an empty list of tweets") {
                let twitterResponses : NSString = "{\"statuses\":[]}"
                let json = JSON(twitterResponses)
                let tweets = json["statuses"].arrayValue.map {Tweet($0)}
                expect(tweets.count).to(equal(json["statuses"].arrayValue.count))
            }
